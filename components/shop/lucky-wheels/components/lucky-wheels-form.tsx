...
export function LuckyWheelForm({ luckyWheel, onChange, ...props }: PropsType) {
  ...
  return (
    <>
      <Form
        ...
        onSubmit={onSubmit}
      >
        <Field name="title" label="Tên vòng quay" cols={8} required>
          <Input />
        </Field>
                          ...
                          {!gift.isLose && (
                            <span
                              className={`status-label self-center bg-${
                                GIFT_TYPES.find((x) => x.value == gift.type)?.color
                              }`}
                            >
