...
export function InstructionsProvider({ onComplete, ...props }: Props) {
  ...
  const handleSubmit = async (data: any) => {
    try {
      switch (step) {
        ...
        case 2:
          {
            await createOrUpdateBranch(data);
          }
          break;
        case 3:
          {
            await StaffService.createOrUpdate({ id: defaultValues?.id, data });
          }
          break;
        case 4:
          {
  ...
  const createOrUpdateBranch = async (data) => {
    ...
    await ShopBranchService.createOrUpdate({ id: defaultValues.id, data: newData });
  };
