...
export function TriggerForm({ trigger, triggerGroupId, triggerGroups, ...props }: Props) {
  ...
  return (
    <>
      <Form
        ...
        {...props}
      >
        <Field name="name" label="Tên chiến dịch" cols={12} required>
          <Input />
        </Field>
        ...
        <Form.Footer
          ...
          cancelText=""
          submitProps={{ className: "h-14 px-16" }}
        />
      </Form>
      <ActionForm
                  ...
                  {action.options.blockId && (
                    ...
                    >
                      {action.options.pagePicture && (
                        <Img
                          lazyload={false}
                          className="grow-0 shrink-0 w-12"
                          rounded
...
function ActionTypeFields({ event }: { event: string }) {
  ...
  return (
    <>
      ...
      {type == "notification" && (
        <Field name="options.title" label="Tiêu đề">
          <Input />
        </Field>
      )}
    ...
    <>
      ...
      {blockId ? (
        <div className="mb-6">
          ...
          >
            {pagePicture && (
              <Img lazyload={false} className="grow-0 shrink-0 w-12" rounded src={pagePicture} />
            )}
            <div className="flex-1 pl-3">
  ...
  return (
    <>
      <div
        {...(onChange
          ? {
              className: `form-control py-2 focus-within:border-primary cursor-text ${className} ${
                error ? "error" : ""
              }`,
              onClick: (e) => {
                if (
