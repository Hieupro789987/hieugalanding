...
function NotificationSettings() {
  ...
  return (
    ...
    >
      ...
      <Form.Footer isReverse={false} />
    </Form>
  );
}

function ManyChatSettings() {
            ...
            <span className="flex items-center ml-1 font-semibold text-gray-700">
              <div
                className={`w-2.5 h-2.5 mr-2 rounded-full bg-${
                  MANYCHAT_STATUS.find((x) => x.value == shopConfig.manychatConfig.status)?.color
                }`}
              ></div>
              <span
                className={`text-${
      ...
      <div className="flex items-center justify-between col-span-12 mb-4">
        <div>
          ...
          <span className="flex items-center ml-1 font-semibold text-gray-700">
            <div
              className={`w-2.5 h-2.5 mr-2 rounded-full bg-${
                ZALO_STATUS.find((x) => x.value == shopConfig.zaloConfig.status)?.color
              }`}
            ></div>
            <span
              className={`text-${
