...
export function RegisterPage() {
  ...
  async function regisCollab() {
    ...
    try {
      ...
      toast.info("Bạn chưa thỏa điều kiện để trở thành Cộng tác viên: " + error.message);
      err = error;
    } finally {
      if (!err) {
        await getCustomer();
  ...
  return (
    <div className="relative w-full min-h-screen bg-white rounded-md shadow">
      <BreadCrumbs
        breadcrumbs={[{ label: "Trang chủ", href: `/store/${shopCode}` }, { label: "Đăng ký CTV" }]}
        className="pt-4"
      />
      <div className="min-h-screen px-4">
        <h2 className="py-2 pt-4 text-lg font-bold text-center text-gray-700 uppercase border-b">
          Điều khoản dịch vụ
        </h2>
        {shop.config.colTerm ? (
          <div
