...
export function Input({
  ...
  const numberMask = useMemo(() => {
    return createNumberMask({
      ...
      decimalLimit: 2,
      ...(!!maxLength && { integerLimit: maxLength }),
    });
  ...
  useEffect(() => {
    if (valueInited && debouncedValue !== undefined && props.onChange) {
      props.onChange(
        debouncedValue,
        number || type == "number"
          ? parseTextToNumber(debouncedValue, decimalSeparator)
          : debouncedValue
      );
    }
  }, [debouncedValue]);
