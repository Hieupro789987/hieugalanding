        ...
        <>
          <div className={`relative w-full overflow-hidden ${tabsClassName}`}>
            ...
            <div
              id={id}
              ref={tabRef}
              className={`text-center relative flex items-center overflow-x-hidden ${
                hasArrow ? "px-4" : ""
              } ${className}`}
            >
              {tabs.map((tab, index) => (
              ...
              {hasInkBar && (
                <div
                  className={`absolute bottom-0 h-1 transition-all duration-300 ease-in-out origin-center ${inkBarClassName}`}
                  ref={inkbarRef}
                ></div>
              )}
            </div>
