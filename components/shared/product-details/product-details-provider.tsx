...
import { calculateProductPriceWithPolicy, PricePolicy } from "../../../lib/repo/price-policy.repo";
import {
...
export function ProductDetailsProvider({
  ...
  if (isDiscountItems) {
    ...
    useEffect(() => {
      setProductCode(
        router.query.product?.toString() || router.query.productCode?.toString() || ""
      );
    ...
    }, [router.query.product, router.query.productCode, router.query.editProduct]);
  ...
  const similarProductCrud = useCrud(
    ProductService,
    {
      limit: 10,
      filter: {
    ...
    ProductService.getAll({
      ...
      .then((res) => {
        if (res.data.length) {
          ...
          newProduct.price = calculateProductPriceWithPolicy({
            policyBestPrice: newProduct.policyBestPrice,
  ...
  return (
    <ProductDetailsContext.Provider
      value={{
        ...
        similarProducts: similarProductCrud.items,
      }}
    >
      {props.children}
    </ProductDetailsContext.Provider>
