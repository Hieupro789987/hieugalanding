...
export function AddressGoogleDialog({ onChange, fullAddress = "", ...props }: Props) {
  ...
  return (
    <Dialog width={600} {...props} headerClass=" ">
      <DialogHeader title="Nhập địa chỉ" onClose={props.onClose} />
      <div
                  ...
                  <>
                    {placePredictions.map((place) => (
                      <button
                        type="button"
                        key={place.place_id}
                        className="flex items-start w-full p-3 text-gray-600 border-b border-gray-200 animate-emerge hover:bg-primary-light"
                        onClick={async () => {
                          setPlace({ ...place });
                        }}
                      >
                        <i className="mt-1 ml-3 mr-1 text-xl">
                          <RiMapPinLine />
                        </i>
